...
class CreateBaseDatabase(Migration):
    def migrate(self):
        ...
        with self.create_tables("profile_layouts") as x:
            x.primary_key("id", auto_increment=True)
            x.string("name", null=False)
        ...
        with self.create_tables("guild_users") as x:
            x.primary_key("id", auto_increment=True)
            x.references("guild", null=False, on_delete="CASCADE")
            ...
            x.references("profile_layout", on_delete="SET NULL")
            x.integer("has_accepted_rules", null=False)
    ...
    def rollback(self):
        ...
        self.drop_join_table(
            "guild_users", "wallpapers", verb="buy"
        )
        self.drop_tables(
            "guild_users",
            "profile_layouts",
