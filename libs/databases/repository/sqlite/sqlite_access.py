...
class SqliteAccess(DatabaseAccessImplement):
    ...
    def add_user_point(self, discord_id: str, point: int = 1) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET point = point + ? WHERE discordId = ?", [point, discord_id])
    ...
    def add_user_level(self, discord_id: str, level=1) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET level = level + ? WHERE discordId = ?", [level, discord_id])
    ...
    def add_smartpoint(self, discord_id, amount=1) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET smartpoint = smartpoint + ? WHERE discordId = ?", [amount, discord_id])
    ...
    def add_user_monthly_point(self, discord_id: str, point: int = 1) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET monthlyPoint = monthlyPoint + ? WHERE discordId = ?", [point, discord_id])
    ...
    def reset_user_monthly_point(self, discord_id: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET monthlyPoint = 0 WHERE discordId = ?", [discord_id])
    ...
    def add_user_if_not_exist(self, discord_id: str) -> None:
        ...
        if not self.__sqliteDB.select("SELECT discordId FROM users WHERE discordId = ?", [discord_id]):
            self.__sqliteDB.modify(
                "INSERT INTO users(discordId) VALUES (?)", [discord_id])
    ...
    def reset_point(self, discord_id: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET point = 0 WHERE discordId = ?", [discord_id])
    ...
    def reset_level(self, discord_id: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET level = 1 WHERE discordId = ?", [discord_id])
    ...
    def set_user_root(self, discord_id: str, is_root: bool) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE users SET isRoot = ? WHERE discordId = ?", [
                               is_root, discord_id])
    ...
    def get_user_current_wallpaper(self, discord_id: str) -> str:
        ...
        """
        query_result = self.__sqliteDB.select(
            "SELECT wallpapers.name FROM users INNER JOIN wallpapers ON users.wallpapersId = wallpapers.id WHERE discordId = ?", [discord_id])
    ...
    def change_user_profile_custom_color(self, discord_id: str, profile_colored_part: ProfileColoredPart, color: str) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE users SET " + profile_colored_part.value +
                               " = ? WHERE discordId = ?", [color, discord_id])
    ...
    def increase_number_of_buy(self, discord_id: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET numberOfBuy = numberOfBuy + 1 WHERE discordId = ?", [discord_id])
    ...
    def reset_number_of_buy(self, discord_id: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "UPDATE users SET numberOfBuy = 0 WHERE discordId = ?", [discord_id])
    ...
    def get_user_profile_layout(self, discord_id: str) -> str:
        ...
        """
        query_result = self.__sqliteDB.select(
            "SELECT profilesLayout.name FROM profilesLayout INNER JOIN users ON users.profilesLayoutId = profilesLayout.id WHERE discordId = ?", [discord_id])
    ...
    def get_profile_layout(self, layout_name: str) -> Layout:
        ...
        """
        raw = self.__sqliteDB.select(
            "SELECT profilPictureX, profilPictureY, nameX, nameY, userNameX, userNameY, levelX, levelY, badgeX, badgeY, levelBarX, levelBarY FROM profilesLayout WHERE name = ?", [layout_name])[0]
    ...
    def add_profile_layout(self, layout_name: str, layout: Layout) -> None:
        ...
        """
        self.__sqliteDB.modify("INSERT INTO profilesLayout(name, profilPictureX, profilPictureY, nameX, nameY, userNameX, userNameY, levelX, levelY, badgeX, badgeY, levelBarX, levelBarY) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
                               layout_name, layout.profile_picture.x, layout.profile_picture.y, layout.name.x, layout.name.y, layout.username.x, layout.username.y, layout.level.x, layout.level.y, layout.badge.x, layout.badge.y, layout.level_bar.x, layout.level_bar.y])
    ...
    def remove_profile_layout(self, layout_name: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "DELETE FROM profilesLayout WHERE name = ? AND id != 1", [layout_name])
    ...
    def update_profile_layout(self, layout_name: str, layout: Layout) -> None:
        ...
        Args:
            ...
            layout (Layout): The profile layout.
        """
        self.__sqliteDB.modify("UPDATE profilesLayout SET profilPictureX = ?, profilPictureY = ?, nameX = ?, nameY = ?, userNameX = ?, userNameY = ?, levelX = ?, levelY = ?, badgeX = ?, badgeY = ?, levelBarX = ?, levelBarY = ? WHERE name = ?", [
                               layout.profile_picture.x, layout.profile_picture.y, layout.name.x, layout.name.y, layout.username.x, layout.username.y, layout.level.x, layout.level.y, layout.badge.x, layout.badge.y, layout.level_bar.x, layout.level_bar.y, layout_name])
    ...
    def rename_profile_layout(self, old_layout_name: str, new_layout_name: str) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE profilesLayout SET name = ? WHERE name = ?", [
    ...
    def add_wallpaper(self, wallpaper_name: str, url: str, price: int, level: int) -> None:
        ...
        """
        self.__sqliteDB.modify("INSERT INTO wallpapers(name, url, price, level) VALUES (?, ?, ?, ?)", [
                               wallpaper_name, url, price, level])
    ...
    def remove_wallpaper(self, wallpaper_name: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "DELETE FROM wallpapers WHERE name = ?", [wallpaper_name])
    ...
    def set_wallpaper_url(self, wallpaper_name: str, url: str) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE wallpapers SET url = ? WHERE name = ?;", [
                               url, wallpaper_name])
    ...
    def set_wallpaper_price(self, wallpaper_name: str, price: int) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE wallpapers SET price = ? WHERE name = ?", [
                               price, wallpaper_name])
    ...
    def set_wallpaper_level(self, wallpaper_name: str, level: int) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE wallpapers SET level = ? WHERE name = ?", [
                               level, wallpaper_name])
    ...
    def rename_wallpaper(self, old_wallpaper_name: str, new_wallpaper_name: str) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE wallpapers SET name = ? WHERE name = ?", [
                               new_wallpaper_name, old_wallpaper_name])
    ...
    def add_role(self, role_discord_id: str, level: int) -> None:
        ...
        Args:
            ...
            level (int): The role level.
        """
        self.__sqliteDB.modify("INSERT INTO roles(discordId, level) VALUES (?, ?)", [
                               role_discord_id, level])
    ...
    def remove_role(self, role_discord_id: str) -> None:
        ...
        """
        self.__sqliteDB.modify(
            "DELETE FROM roles WHERE discordId = ?", [role_discord_id])
    ...
    def update_role_level(self, role_discord_id: str, level: int) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE roles SET level = ? WHERE discordId = ?", [
                               level, role_discord_id])
    ...
    def set_user_accepted_rules(self, discord_id: str, accepted: bool) -> None:
        ...
        """
        self.__sqliteDB.modify("UPDATE users SET acceptedRules = ? WHERE discordId = ?", [
                               accepted, discord_id])
