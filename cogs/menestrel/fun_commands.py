...
class FunCommands(discord.Cog):
    ...
    async def rock_paper_scissors(self, ctx: discord.ApplicationContext, choice: str):
        Log.command(ctx.author.name +
                    " is launching rock_paper_scissors commands")
        try:
            ...
            bot_choice_emoji = possibilities_emojis[possibilities.index(
                random_choice)]
            user_choice_emoji = possibilities_emojis[possibilities.index(
                choice)]
            ...
            if winner_id == -1:
                ...
                message += f"Winner is <@{winner_id}> :trophy: !"
